...
6. Условное изображение категории будет планарной геометрией. Поместим созданные кривые в планарную геометрию с помощью метода ``AddCurves`` и добавим основную заливку с помощью метода ``AddMaterialColorSolidArea``.

.. code-block:: lua
    :caption: Создание планарной геометрии.
    :linenos:
...
Cозданную плоскую геометрию ``geometry`` добавляем в модельную геометрию ``symbolicGeometry`` с помощью метода ``AddGeometrySet2D``.
